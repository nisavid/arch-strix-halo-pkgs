...
compressed-tensors 0.18.0, and llmcompressor 0.13.0 are rejected for this
convergence line; a separate active candidate owns the required
...
The production adapter contract fails closed. No production signer, immutable
evidence store, append-only journal, rollback-resistant register/quorum,
witness roster, target fence, recovery root, or trusted-time source is selected
by repository code. The operator-approved production substrate topology remains
open in issue #104, so this work does not establish W0 authority or permit a
...
`lemonade-server 10.6.0-9`. Deployed/installed: complete for those built
packages; `pacman -Q` reports each expected package version. Local-repo
verified: the local repo contains the matching archives. Published-repo
verified: the b9442 handoff recorded matching `strix-halo-gfx1151` sync
database entries for both b9442 backends and `lemonade-server 10.6.0-9`.
Installed-smoked: complete for AutoRound import, compressed-tensors import,
llama.cpp HIP and Vulkan server wrapper version checks, stable-diffusion.cpp
Vulkan CLI and server wrapper startup, Lemonade CLI startup, and Lemonade b9442
backend metadata. Live-scenario validated: complete for
...
The ROCm PyTorch release/2.12 `26872de` runtime-base source/build/deploy/live
...
`2.12.0-2`. The 2026-05-28 targeted closeout check also found Arch
`python-pytorch-opt-rocm 2.12.0-2`; the local package version is aligned with
that pkgrel while the source pin remains the repo-owned ROCm branch target. The
package keeps the existing local ROCm/gfx1151 patch semantics refreshed against
that source and carries
...
The scenario stdout proved FlashAttention CK backend import against
`flash_attn 2.8.4`, Torch-MIGraphX PT2E quantizer importability, and vLLM
...
The llama.cpp b9357 refresh is adopted. Both packaged backends now render from
...
`llama.cpp-vulkan-gfx1151 b9357-1`. Deployed/installed: complete for b9357;
read-only installed-package verification reports `lemonade-server 10.6.0-8`,
`llama.cpp-hip-gfx1151 b9357-1`, and
`llama.cpp-vulkan-gfx1151 b9357-1`. Installed-smoked: complete for b9357; the
installed help scenarios `llama.cpp.hip.help`, `llama.cpp.vulkan.help`,
`lemonade.cli.help`, and `lemonade.server.help` passed with zero failures at
`docs/worklog/inference-runs/20260528T-llama-b9357-postdeploy-help`.
Live-scenario validated: complete for b9357; the installed
`lemonade.reranking.zerank-2.selected-logit` scenario passed with zero failures
at `docs/worklog/inference-runs/20260528T-llama-b9357-postdeploy-live`.

After recording b9357 source/build/install/live progress, a targeted
...
The active package ledger set is four tracked follow-ups plus one blocked
candidate: llama.cpp b9444 source follow-up, ROCm PyTorch release/2.12
`f8efdb3` follow-up, vLLM 0.22.0 follow-up, Transformers 5.9.0 follow-up, and
the AITER 0.1.15-rc0 dependency-closure blocker. Each active entry has its
...
tracked work. AutoRound 0.13.0, compressed-tensors 0.16.0, and
stable-diffusion.cpp be65ac7 are adopted in the ledger after deploy/install and
installed smokes.
...
candidate-ledger state for llama.cpp b9352, stable-diffusion.cpp 92dc726, the
initial ROCm PyTorch release/2.12 980ce60 observation, and Transformers 5.9.0.
...
installed-smoke, and live-scenario gates before adoption. The 2026-05-31
...
generic `gfx1151` target references, but no top-level Meta xFormers build or
test lane advertises `gfx1151`, Meta ROCm wheel CI still targets `gfx90a
gfx942`, and public PyPI carries only a generic Linux wheel plus sdist for
`0.0.35`. The ROCm fork remains unreleased at `develop`
`6b467648906fd0317c61e6ad1bc27a2ed14df17e`; its top-level build path accepts
`gfx11`/`gfx12` architecture strings and its recursive submodules contain
`gfx1151` scout references, but its wheel/build workflows still target `gfx90a
gfx942` and do not publish a `gfx1151` package lane. Repo-owned
package/scenario surfaces have no current xFormers consumer. Reopen package
planning only after a named consumer and explicit package-level `gfx1151`
source-build proof exist, then require pinned source and submodules,
...
claimed for this review. The exact reopen criteria live in
...
Watchfiles 1.2.0 is adopted and installed as
`python-watchfiles-gfx1151 1.2.0-1`. Upstream watchfiles 1.2.0 drops Python 3.9
support, adds Python 3.15 classifiers, raises the Rust MSRV to 1.83, and
updates PyO3 to 0.28.3 while keeping the runtime dependency on AnyIO. Arch
`python-watchfiles` remains at `1.1.1-3`, so no Arch baseline carry is adopted
for this slice. `makepkg --verifysource` and
`makepkg --nobuild --nodeps --force` passed for
`packages/python-watchfiles-gfx1151`, `tools/amerge` plan `2973e241` built the
package, and deploy/install plan `a2428587` completed. `pacman -Q
python-watchfiles-gfx1151` reports `python-watchfiles-gfx1151 1.2.0-1`; the
installed import smoke confirms `watchfiles.__version__ == "1.2.0"`, `Change`
enum access, `_rust_notify` import, and callable `watch` and `awatch`.
...
`XDG_CONFIG_HOME`, `npm_config_cache`, and `npm_config_store_dir` into
...
Deploy/install and installed-smoke gates passed on 2026-05-17. The reference
host reports `python-vllm-rocm-gfx1151 0.21.0-1`,
`python-amd-aiter-gfx1151 0.1.14rc1.dev27+g7cfe51983-1`,
`llama.cpp-hip-gfx1151 b9165-1`, `llama.cpp-vulkan-gfx1151 b9165-1`, and
`stable-diffusion.cpp-vulkan-gfx1151 r604.g0b82969-1` from `pacman -Q`.
Installed smokes passed for `vllm --version`, `vllm` Python import, AITER
