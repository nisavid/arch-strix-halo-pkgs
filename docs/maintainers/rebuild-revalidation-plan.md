...
This historical plan captured the workflow used to close
`docs/maintainers/rebuild-revalidation.md`. It assumes the self-hosted native
rebuild is complete and processes revalidation in the package root order
reported by:
...
Use Hugging Face model cards, vLLM issues and PRs, vLLM commit history,
release notes, and current vLLM docs as hints for specific features, expected
backend markers, and failure signatures. Especially useful vLLM references
include:
...
vLLM recipe coverage is no longer part of the rebuild revalidation closeout.
Use `docs/maintainers/vllm-recipe-coverage.md` for the Gemma and Qwen recipe
surface matrix, planned local probes, advisory-only deployment shapes, and
stress-lane candidates.
