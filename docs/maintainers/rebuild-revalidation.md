...
| pending | AOCL installed runtime smoke gap | Backlog item for `aocl-libm-gfx1151` installed-host coverage | AOCL has package/build validation, but no tracked installed-host smoke equivalent to the llama.cpp and Lemonade entrypoint scenarios. | Add a tiny installed-host runtime smoke that proves AOCL-LibM headers and library resolution work from the packaged install, then run it after the rebuilt package is installed. | `docs/backlog.md` and scenario/test docs |
