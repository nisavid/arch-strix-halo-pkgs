...
| --- | --- | --- | --- | --- | --- |
| pending | SentencePiece closure | `python-sentencepiece-gfx1151/0001-bundle-sentencepiece-by-default.patch` | The earlier Gemma 4 tokenizer failure was traced to an installed extension still linked against stale host `sentencepiece`, `protobuf`, and `abseil` libraries. Current package-local docs already identify the rebuilt package artifact as self-contained. | Treat the package-local ELF test as the retained guard, then rerun at least one Gemma 4 tokenizer/vLLM smoke that imports SentencePiece through the rebuilt installed stack before promoting any runtime conclusion. | `docs/patches.md` or `packages/python-sentencepiece-gfx1151/README.md` |
