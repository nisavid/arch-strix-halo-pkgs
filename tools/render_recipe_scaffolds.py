...
def lemond_unit_check_snippet() -> str:
  ...
  if [[ $(grep -cE '^[[:space:]]*ExecStart[[:space:]]*=' "${_unit}") -ne 1 ]] ||
    grep -qE '^[[:space:]]*Environment[[:space:]]*=.*(LEMONADE_CACHE_DIR|HOME|XDG_[A-Z_]+)=' "${_unit}"; then
...
build() {{
  ...
  local _clean_env=()
  local _env_name
  while IFS='=' read -r -d '' _env_name _; do
    case "${{_env_name}}" in
