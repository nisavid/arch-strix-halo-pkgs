...
- Authoritative base: Arch split package python-pytorch-opt-rocm 2.12.0-1, with python-pytorch-rocm kept as an advisory sibling because the same pkgbase carries both variants.
...
- Keep ROCm BLAS GEMM/BGEMM template specializations exported from libtorch_hip.so so same-library ROCm native objects can resolve their dynamic references at import time.

## Intentional Divergences

- Uses the Arch split-package structure as the integration baseline but deliberately tracks ROCm/pytorch release/2.12 rather than upstream pytorch/pytorch.
...
- On 2026-05-31, reviewed ROCm/pytorch release/2.12 through f8efdb30b5e6d8b1ae3e8744f227416b2aa032c1. The 26872de..f8efdb range includes the large-matrix triu/tril 64-bit indexing fix, the UnaryUfuncInfo lambda syntax fix, and a ROCm foreach profiler-test skip. Keep this as a tracked follow-up until the source pin, affected rebuild set, deploy/install, installed smoke, and live scenario gates are completed.
- On 2026-06-15, adopt ROCm/pytorch release/2.12 at c7badbdf3d33d945a0ed4536aac5303bc933e6ee as the canonical runtime-base source lane. The f8efdb..c7badbdf range changes test coverage and NCCL symmetric-memory device-support gating without touching the local patch paths; use pkgrel 4 to align with Arch python-pytorch-opt-rocm 2.12.0-4 and to supersede the unmerged ab32a1f/pkgrel-3 host drift.
...
- Keep the ROCm BLAS template-specialization export patch while release/2.12 builds libtorch_hip.so with hidden default visibility; without it, torch import can fail on unresolved at::cuda::blas::gemm symbols.
