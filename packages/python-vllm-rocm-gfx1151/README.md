...
- Does not carry the broader fused-MoE default-policy flip. The 2026-04-17 reference-host `google/gemma-4-26B-A4B-it` text smoke faulted the GPU during AITER fused-MoE 2-stage initialization when that policy forced the path on without an explicit runtime override, so the durable carry stays limited to the narrower Gemma 4 repair lane.
