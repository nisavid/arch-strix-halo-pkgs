...
- Applied source patch files/actions: `36`
...
- The current repo-owned `google/gemma-4-26B-A4B-it` validation lane is tracked through `tools/gemma4_text_smoke.py` and `tools/gemma4_server_smoke.py`. It is text-only basic serving with `--limit-mm-per-prompt {"image":0,"audio":0,"video":0}`, `--max-model-len 128`, and `--max-num-batched-tokens 32`, and it intentionally keeps eager mode on as a correctness lane until the compiled/cudagraph ROCm path is revalidated separately. The 2026-04-19 E2B compiled investigation did not promote compiled mode: installed Triton first failed torch.compile with a missing `AttrsDescriptor.__repr__`, a temporary repr shim let the compiled+cudagraph path initialize but produce corrupted text, and a no-cudagraph compiled probe faulted the GPU during initialization/warmup.
- The wider Gemma 4 scenario matrix is tracked under `inference/scenarios/vllm-gemma4.toml`. Broad scenario selections skip entries tagged `exploratory` by default, so the multimodal, compiled-path, forced-MoE-backend, and real-model TorchAO probes remain opt-in until reference-host runs promote them.
...
- `tools/torchao_vllm_smoke.py` now covers both the tiny no-download checkpoint and a real-model path that quantizes `--source-model` with TorchAO int8 weight-only before serving it through vLLM. Treat the real-model scenario as exploratory until its warning surface and memory profile are captured on the reference host.
...
- Carries a repo-owned Gemma 4 26B-A4B text-only validation lane for Strix Halo because the host-proven path is narrower than upstream defaults: AITER unified attention, TRITON unquantized MoE, and eager-mode execution.
...
- Treat the imported Blackcat Qwen3.5 hybrid-attention/GDN notes as advisory only until repo-owned validation proves which hybrid guards, FLA fixes, and AITER exclusions still matter on this packaged v0.19.0 lane. The current maintained patch carry does not yet include a repo-owned Qwen3.5 hybrid/GDN validation story.
- Do not restore the dropped Gemma 4 ROCm AITER MoE intermediate-padding carry unless a future explicit AITER fused-MoE lane reproduces a real alignment failure after the AITER backend is re-enabled on purpose. The 2026-04-17 validated reference-host lane used TRITON backend for Unquantized MoE, so that patch was removed as dormant carry.
