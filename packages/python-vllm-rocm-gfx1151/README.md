...
- Carries a repo-owned Qwen3.5 hybrid/GDN patch lane for the parts of Blackcat Informatics' advisory recipe that are still missing from the maintained vLLM 0.19.0 package source.
...
- Keep `0010-rocm-support-qwen35-hybrid-gdn.patch` until upstream vLLM lands equivalent ROCm-safe Qwen3.5/GDN handling. The patch carries the still-missing pieces from Blackcat Informatics' advisory lane: AMD-restricted FLA autotune grids, float32 GDN exponent operands, GDN warmup at T=64 only, hybrid block-size realignment after ROCm platform updates, and hybrid full-attention fallback away from AITER attention. In vLLM 0.19.0, the GDN warmup loop lives in `vllm/model_executor/layers/mamba/gdn_linear_attn.py`, not the older advisory `qwen3_next.py` path.
