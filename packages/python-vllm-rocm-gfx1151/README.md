...
- Keep the repo-owned `google/gemma-4-26B-A4B-it` text-only smoke lane on `enforce_eager=True` / `--enforce-eager` until a separate host validation proves the compiled/cudagraph ROCm path is clean. vLLM documents eager mode as disabling compilation and cudagraph capture, so treat relaxing it as a follow-up performance lane rather than a silent default change.
